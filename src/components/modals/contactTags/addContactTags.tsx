...
interface AddContactTagModalProps {
  onClose: () => void;
  onUpdate: () => void;
}
...
export default function AddContactTagModal({
  onClose,
  onUpdate,
}: AddContactTagModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await axios.post("/api/contact-tags", { name, color });
      toast({
        title: "Tag created successfully",
        description: `The ${name} tag has been added`,
      });
      onUpdate();
      onClose();
    } catch (error) {
      ...
      console.error("Failed to create contact tag:", error);
    } finally {
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      <div className="space-y-4">
        ...
        <div className="space-y-3">
          ...
          <div className="h-32 ss rounded-md border">
